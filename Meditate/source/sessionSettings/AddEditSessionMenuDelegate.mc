...
class AddEditSessionMenuDelegate extends Ui.Menu2InputDelegate {
	...
	function onSelect(item) {
		...
		if (id == :time) {
			...
			var vibeMenu = new Ui.Menu2({ :title => Ui.loadResource(Rez.Strings.vibePatternMenu_title), :focus => focusIdx });
			vibeMenu.addItem(
			...
			var focusIdx = 0;
			if (atVal == ActivityType.Yoga) { focusIdx = 1; }
			else if (atVal == ActivityType.Breathing) { focusIdx = 2; }
			else if (atVal == ActivityType.Generic) { focusIdx = 3; }
